...
class Hd44780Lcd {
 public:
  ...
  // CGRAM write + immediate restore of DDRAM addressing. After this returns,
  // the LCD's address pointer is back at DDRAM(0, 0) so subsequent buffered
  // writes don't accidentally land in CGRAM. Caller must Flush() the
  // queue before calling and invalidate the BufferedDisplay's
  // position-tracker after, to force a MoveCursor on the next tick.
  static inline void SetCustomCharMapAndRestore(
      const uint8_t* data,
      uint8_t num_characters,
      uint8_t first_character) {
    SlowCommand(LCD_SET_CGRAM_ADDRESS | (first_character << 3));
    ...
    SlowCommand(LCD_SET_DDRAM_ADDRESS | 0);
  }

  // Queued CGRAM write — pushes the commands through the same OutputBuffer
  // the BufferedDisplay uses, so the timer ISR consumes them in order
  // without any interrupts-off window or busy-wait. The trailing
  // SET_DDRAM_ADDRESS command restores screen-addressing mode so the
  // BufferedDisplay's subsequent DDRAM writes hit the screen.
  // Returns 1 on success, 0 if the buffer didn't have room for the full
  // sequence (in which case nothing was queued — caller retries later).
  // Each LCD op = 2 nibbles = 2 buffer bytes. Sequence is:
  //   1 cmd (CGRAM addr) + num_characters*8 data + 1 cmd (DDRAM restore)
  // = (2 + num_characters * 8) * 2 buffer bytes.
  static inline uint8_t QueueCustomCharMap(
      const uint8_t* data,
      uint8_t num_characters,
      uint8_t first_character) {
    uint8_t needed = (2 + num_characters * 8) * 2;
    if (OutputBuffer::writable() < needed) {
      return 0;
    }
    WriteCommand(LCD_SET_CGRAM_ADDRESS | (first_character << 3));
    for (uint8_t i = 0; i < num_characters * 8; ++i) {
      WriteData(*data++);
    }
    WriteCommand(LCD_SET_DDRAM_ADDRESS | 0);
    return 1;
  }

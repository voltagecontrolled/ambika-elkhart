...
class BufferedDisplay {
 public:
  ...
  // Force the next Tick to re-issue a MoveCursor before its WriteData.
  // Needed after any direct LCD command (e.g. SetCustomCharMap) that
  // bypasses the buffered write queue and disrupts the LCD's expected
  // address pointer.
  // NOTE: we deliberately do NOT touch remote_ here. Clearing remote_
  // forces a full 80-char screen rewrite over many Ticks and produces
  // a visible flicker each time. Resetting scan_position_last_write_
  // alone is sufficient: the BufferedDisplay::Tick contiguous-write
  // optimization only fires when scan_position_ == prev + 1, which
  // never matches 255 + 1 (= 0) since column 0 always re-issues
  // MoveCursor anyway.

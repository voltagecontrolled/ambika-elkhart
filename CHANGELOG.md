...
### v4.4 (in progress) — LFO 4 clock sync, one-shot shapes, LFO 5, env/LFO page reorg, pre-filter wavefolder, sn16 sine bank, intrinsic locks unified, WS1 lock-surface unification (2026-05-23)

**Flash:** controller 90.3% → **93.8%** (`+1548 B` over v4.3). **RAM:** controller 79.6% → **81.6%** (`+82 B` net; +120 B for the WS1 `defaults[]` growth, offset by other reclaims). Voicecard 80.0% → **81.4%** (`+437 B` flash, `+30 B` RAM) — wavefolder + sn16 + WS1 snapshot extension; the sn16 swap nets out the CZ shape removal. `kSystemVersion` left at v4.3 levels per mid-sprint policy; bumps when v4.4 closes. Snapshot file format bumps to `0x07`; v0x06 → v0x07 migration seeds the new `defaults[28..47]` from PROGMEM `kDefaultExt` (matches a freshly initialized track for those bytes). v4.1/v4.2/v4.3/v4.4-mid (`0x01`..`0x05`) snapshots still load via the existing migration path with the same tail seed.

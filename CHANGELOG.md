...
### v4.4 (in progress) — LFO 4 clock sync, one-shot shapes, LFO 5, env/LFO page reorg, pre-filter wavefolder, sn16 sine bank, intrinsic locks unified (2026-05-23)

**Flash:** controller 90.3% → **91.1%** (`+556 B` over v4.3). **RAM:** controller 79.6% → **78.2%** (`−56 B`). Voicecard 80.0% → **81.3%** (`+416 B` flash, `+10 B` RAM) — the wavefolder + sn16 net out −108 B from the prior v4.4 voicecard size by removing 9 CZ filter-sim shape render bodies. `kSystemVersion` left at v4.3 levels per mid-sprint policy; bumps when v4.4 closes. Snapshot file format bumps to `0x06`; v4.1/v4.2/v4.3/v4.4-mid (`0x01`..`0x05`) snapshots still load — old snapshots' diverged intrinsic bytes come back as pool-backed locks (see "intrinsic locks unified" below).

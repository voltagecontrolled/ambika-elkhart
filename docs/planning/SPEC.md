...
> PITD/PITA pitch envelope). The shipped architecture diverged: three independent
> ADR+Curve envelopes (no sustain), fixed mod routing, LFO1/2/3 removed from the
> controller side, `patch_mod[]` removed from `SeqTrack`. **For the as-built
> design, read `../../CHANGELOG.md` first** and the topic specs `voice_envelopes.md`
> and `sequencer.md` in this directory. Use this file only as historical
> reference for design intent, or as a target to converge toward when scope
> allows.
>
> *(Renamed from `SPEC_v2.md` — the "v2" framing referred to the second design
> iteration after the Carcosa branch was abandoned and is no longer
> load-bearing.)*
